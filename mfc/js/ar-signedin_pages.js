...
function redirectSignedOutUser() {
  firebase.auth().onAuthStateChanged(user => {
    if (!user) {
      ...
      window.location.href = `ar-signin?directURL=${encodedCurrentURL}`;
    }
  });
}
...
async function redirectToCompleteProfileInfo(email) {
  ...
  try {
    ...
    if (directURL) {
      ...
      window.location.href = `ar-complete-info?email=${email}&token=${token}&directURL=${encodedCurrentURL}`;
    } else {
      curentURL = window.location.pathname.split("/").pop();
      window.location.href = `ar-complete-info?email=${email}&token=${token}&directURL=${curentURL}`;
    }
  } catch {  
...
async function redirectToCompleteProfile(email) {
  ...
  try {
    ...
    if (directURL) {
      ...
      curentURL = window.location.pathname.split("/").pop();
      window.location.href = `ar-complete-profile?email=${email}&token=${token}&directURL=${curentURL}`;
    }
  } catch (error) {  
